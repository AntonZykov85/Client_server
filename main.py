...
def get_data():
    ...
    for i in range(1,4):
        ...
        os_prod_reg = data[data.find('Изготовитель системы:') + 1:]
        os_prod_list.append(os_prod_reg)

        # Название ОС
        os_name_reg = re.compile(r'Название ОС:\w*.\w*')
        os_name_list.append(os_name_reg.findall(data))
...
def write_to_csv(out_file):
    ...
    main_data = get_data()
    with open(out_file, 'w', encoding='Windows-1251') as file:
