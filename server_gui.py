...
def gui_create_model(database):
    ...
    list_ = QStandardItemModel()  # Это начинка
    ...
    for row in list_users:
        ...
        user.setEditable(False)  # запрет на редактирование
        ip = QStandardItem(ip)
...
class HistoryWindow(QDialog):
    # QWidget в QDialog - нет развертывания и скрытия только 2 кнопки скарыть и ?
    def __init__(self):
        super().__init__()
        self.initUI()
